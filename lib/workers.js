...
workers.validateCheckData = function (originalCheckData) {
  ...
  originalCheckData.timeoutSeconds =
    ...
    originalCheckData.timeoutSeconds <= 5
      ? originalCheckData.timeoutSeconds
      : false;
  originalCheckData.state = originalCheckData.state =
...
workers.performCheck = function (originalCheckData) {
  ...
  var parserUrl = url.parse(
    `${originalCheckData.protocol}://${originalCheckData.url}`,
    true
  );

  var hostName = parserUrl.hostname;
  var path = parserUrl.path;
  ...
  var requestDetails = {
    ...
    path: path,
    timeout: originalCheckData.timeoutSeconds * 100,
  };
...
workers.alertUserToStatusChange = function (newCheckData) {
  ...
  helpers.sendTwilioSms(newCheckData.userPhone, msg, function (err) {
    if (!err) {
      ...
      debug(
        "Error: Could not send sms alert to user who had a change in their check",
        err
      );
    }
  });
